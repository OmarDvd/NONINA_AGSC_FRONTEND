...
export function FormularioCreateHookEventEdit({ toggleState}) {
  ...
  useEffect(() => {
    ...
    if (eventID) {
      fetch(`https://noninabackendapigr.work.gd/api/Evento/${eventID}`)
  ...
  const editList = async (data) => {
    ...
    try {
      ...
      const response = await fetch('https://noninabackendapigr.work.gd/api/Evento/UpdateEventoToken?Id='+eventID
